...
// ===== This Week (from data/github-weekly.json, generated by the Monday
// GitHub Action — see .github/workflows/update-github-data.yml) =====
async function renderThisWeek() {
    ...
    try {
        const res = await fetch('data/github-weekly.json', { cache: 'no-store' });
        if (!res.ok) {
            throw new Error(`HTTP ${res.status}`);
        }
        const data = await res.json();
        const { updatedRepos = [], newReleases = [], generatedAt } = data;

        if (updatedRepos.length === 0 && newReleases.length === 0) {
            container.innerHTML = '<p class="section-subtitle">No public repo activity in the last 7 days.</p>';
            return;
        }

        const releasesHtml = newReleases.length ? `
            <div class="this-week-column">
                <h3 class="this-week-column-title">🚀 Releases</h3>
                <ul class="this-week-list">
                    ${newReleases.map(r => `
                        <li>
                            <a href="${r.url}" target="_blank">${escapeHtml(r.name)}</a>
                            <span class="this-week-meta">${escapeHtml(r.tag || '')} &middot; ${formatRelativeDate(r.publishedAt)}</span>
                        </li>
                    `).join('')}
                </ul>
            </div>
        ...
        const updatedHtml = updatedRepos.length ? `
            <div class="this-week-column">
                <h3 class="this-week-column-title">🔨 Updated Projects</h3>
                <ul class="this-week-list">
                    ${updatedRepos.map(r => `
                        <li>
                            <a href="${r.url}" target="_blank">${escapeHtml(r.name)}</a>
                            <span class="this-week-meta">pushed ${formatRelativeDate(r.pushedAt)}</span>
                        </li>
                    `).join('')}
                </ul>
            </div>
        ...
        container.innerHTML = `
            <div class="this-week-grid">${releasesHtml}${updatedHtml}</div>
            <p class="this-week-generated">Generated ${formatRelativeDate(generatedAt)} &middot; updates every Monday</p>
        `;
    } catch (err) {
        console.error('Failed to load This Week data:', err);
        container.innerHTML = '<p class="section-subtitle">This week\'s activity is temporarily unavailable.</p>';
    }
}
