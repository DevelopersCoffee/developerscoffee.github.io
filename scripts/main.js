...
const CONFIG = {
    ...
    // "All Public Repositories" is capped so the grid stays scannable —
    // ranked by a freshness+engagement score, not just raw list order.
    maxReposShown: 12
};
...
// Adapts our weekly-cron JSON shape (data/github-repositories.json,
// generated by scripts/update_github_data.py) to the raw GitHub API repo
// shape the render functions below already expect, so nothing else in
// this file needs to change.
function adaptCronRepo(r) {
    return {
        name: r.name,
        ...
        open_issues_count: r.openIssues || 0,
...
// Freshness + engagement score: recent activity matters more than raw
// star count, but a well-starred/forked repo that's gone quiet still
// ranks above a fresh repo nobody's touched. Recency decays on a ~30-day
// half-life so "pushed today" clearly outranks "pushed 3 months ago".
function engagementScore(repo) {
    const engagement = (repo.stargazers_count || 0) * 3 + (repo.forks_count || 0) * 2;
    const pushed = repo.pushed_at ? new Date(repo.pushed_at).getTime() : 0;
    const daysSincePush = pushed ? (Date.now() - pushed) / 86400000 : 9999;
    const recencyWeight = 1 / (1 + daysSincePush / 30);
    return engagement * recencyWeight + recencyWeight * 5;
}

async function fetchRepositories() {
    try {
        // Primary source: data/github-repositories.json, regenerated every
        // Monday by .github/workflows/update-github-data.yml. Falls back to
        // a live (unauthenticated) GitHub API call if that file is missing
        // or stale-fetch fails, so the page still works before the first
        // cron run or when served from somewhere without data/.
        let repos;
        try {
            const cronResponse = await fetch('data/github-repositories.json', { cache: 'no-store' });
            if (!cronResponse.ok) {
                throw new Error(`HTTP ${cronResponse.status}`);
            }
            const cronData = await cronResponse.json();
            repos = (cronData.repositories || []).map(adaptCronRepo);
        } catch (cronError) {
            console.warn('data/github-repositories.json unavailable, falling back to live API:', cronError);
            const orgResponse = await fetch(`${CONFIG.apiUrl}/orgs/${CONFIG.githubOrg}/repos?sort=updated&per_page=100`);
            if (!orgResponse.ok) {
                throw new Error('Failed to fetch repositories');
            }
            repos = await orgResponse.json();
        }

        // Filter out excluded repositories, forks, and archived repos
        const filteredRepos = repos.filter(repo =>
            !repo.fork && !repo.archived && !CONFIG.excludedRepos.includes(repo.name)
        );

        // Add custom products (dbsync, consentmp) — not real GitHub repos
        ...
        // Return at least custom products if everything else fails
...
function renderRepositories(repos, totalCount) {
    ...
    const remaining = (totalCount || repos.length) - repos.length;
    const moreNote = remaining > 0
        ? `<p class="repos-more-note" style="grid-column: 1/-1;">Showing top ${repos.length} of ${totalCount} by recent activity + stars/forks. <a href="https://github.com/${CONFIG.githubOrg}" target="_blank">See all on GitHub →</a></p>`
        : '';

    container.innerHTML = cardsHtml + moreNote;
}
...
function filterRepositories(filter) {
    ...
    // Only cap the unfiltered "All" view — an active filter/search means
    // the user wants specific matches, not just the top N by score.
    const capped = (filter === 'all' && !searchTerm)
        ? filtered.slice(0, CONFIG.maxReposShown)
        : filtered;

    renderRepositories(capped, filtered.length);
}
...
async function init() {
    ...
    try {
        ...
        renderRepositories(repos.slice(0, CONFIG.maxReposShown), repos.length);
